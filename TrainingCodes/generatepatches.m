...
stride     = 10;  % control the number of image patches
patchsize  = 35;
...
numBatches = 4000;
maxBatches = batchSize * numBatches;
...
ns       = 1;
nscales  = min(1,0.45 + 0.1*randi(5,[ns,nimages]));
...
numPatches  = min(ceil(count/batchSize)*batchSize, maxBatches);
...
count = 0;
imdb.inputs  = zeros(patchsize, patchsize, nch, numPatches,'single');
    ...
    for j = 1: size(nscales,1)
        ...
        for x = 1+step1 : stride : (hei-patchsize+1)
            for y = 1+step2 : stride : (wid-patchsize+1)
                ...
                imdb.inputs(:, :, :, count) = subim_label;
                if count<=diffPatches
                    imdb.inputs(:, :, :, end-count+1)   = HR_current(x : x+patchsize-1, y : y+patchsize-1,1:nch);
                end
            end
        end
    end
